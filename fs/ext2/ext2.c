...
int
ext2_find_inote(ext2_t *ext2_fs, uint32_t inode_id,
...
void
ext2_dir_iterator_init(ext2_t *fs, ext2_inode_t *inode,
...
int ext2_make(ext2_t *fs, uint16_t type,
...
int ext2_read_file_block_doubly(ext2_t *fs, ext2_inode_t *inode, uint32_t block_no, bool create, uint8_t *buff) {
    uint32_t max_blocks = fs->block_size / sizeof(uint32_t);
    uint32_t *blk = (uint32_t *) kmalloc_paging(fs->block_size, NULL);
    if (inode->i_doubly_block == 0) {
        if (create) {
            CHK(ext2_alloc_block(fs, &inode->i_doubly_block, true), "");
        } else return -1;
    }
    CHK(ext2_read_block(fs, inode->i_doubly_block, 1, blk), "");
    uint32_t sblkno = ((block_no - EXT2_SIND_BLOCK) / max_blocks) - 1;
    uint32_t no = (block_no - EXT2_SIND_BLOCK) % max_blocks;
    uint32_t sblkaddr = blk[sblkno];
    if (sblkaddr == 0) {
        if (create)TODO;
        else return -1;
    }
    CHK(ext2_read_block(fs, sblkaddr, 1, blk), "");
    CHK(ext2_read_block(fs, blk[no], 1, buff), "");
    kfree(blk);
    return 0;
    _err:
    if (blk)kfree(blk);
    deprintf("Unexpected exception.");
    return -1;
}
...
int ext2_read_file_block(ext2_t *fs, ext2_inode_t *inode, uint32_t block_no, bool create, uint8_t *buff) {
    ...
    if (block_no < EXT2_SIND_BLOCK) {
        ...
        //singly Linked
        dprintf("singly read[%x]:%x", block_no, inode->i_block[block_no]);
        return ext2_read_file_block_singly(fs, inode, block_no, create, buff);
    } else if (block_no < EXT2_SIND_BLOCK + max_blocks + max_blocks * max_blocks) {
        //doubly linked
        dprintf("doubly read[%x]:%x", block_no, inode->i_block[block_no]);
        return ext2_read_file_block_doubly(fs, inode, block_no, create, buff);
    }
}
 ...
 * @return read bytes*/
int
ext2_read_file(ext2_t *fs, ext2_inode_t *inode,
...
int
ext2_update_inode(ext2_t *fs, uint32_t inode_id,
...
int32_t
ext2_write_file(ext2_t *fs, ext2_inode_t *inode,
                uint32_t offset, uint32_t size, uint8_t *buff) {
    uint32_t block_size = fs->block_size;
    uint32_t block_offset = offset / block_size;
    uint32_t block_count = size / block_size;
